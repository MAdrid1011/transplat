...
基于 C-Tiling 和 DRAM 时序的保守估计：

```
参数：
- tile_C = 8, tiles_per_sample = 16
- Row hit rate: 70% (几何感知调度)
- t_tile ≈ 9 cycles (ping-pong 隐藏延迟)
- t_sample = 16 × 9 = 144 cycles

512 banks 并行：
- 吞吐: 1.07G samples/sec
- TransPlat (4M samples): ~3.7 ms
- 加速比: ~5× (vs GPU 19.65 ms)
```
...
| 指标 | GPU baseline | GeoPIM | 改进 |
|------|--------------|--------|------|
| 几何采样延迟 | 19.65 ms | ~3.7 ms | **5×** |
| 中间数据量 | ~2 GB | ~4 MB | **500×** |
| HBM 外部带宽 | ~4 GB | ~20 MB | **200×** |
...
GeoPIM 的统一"预计算权重"路径支持：

| 采样策略 | 权重来源 | GeoPIM 支持 |
|----------|----------|-------------|
| 极线采样 | GPU (Q-K→softmax) | ✓ |
| 重投影采样 | GPU (MLP 预测) | ✓ |
| 重投影 + 偏移 | GPU (MLP 预测) | ✓ |
